...
class Router {
  ...
  use() {
    ...
    if (typeof args[0] !== 'string') {
      this.wares.push(args[0]);
    } else if (args[0] === '/') {
      this.wares.push(args[1]);
    } else {
      const ware = args[1];
      if (ware.handle) {
        const handles = [ware.handle.bind(ware)];
        this.stack.push(buildRoute('*', args[0], handles, ware));
      } else {
        const handles = [ware];
        this.stack.push(buildRoute('*', args[0], handles));
      }
    }
    return this;
  }
  ...
  //   // this.wares.push(wrap(ware).bind(ware));
  //   return this;
  // }

  // mount(pattern, app) {
  //   const handles = [app.handle.bind(app)];
...
const buildRoute = (method, pattern, handles, app = false) => ({
  method,
  pattern,
  handles,
  app
